...
public class HashTest {
    ...
    public static void main(String[] args) {
        ...
        if(debugLevel == 1){
            try {
                ...
                FileWriter linearWriteFile = new FileWriter("./linear-dump");
                FileWriter doubleWriteFile = new FileWriter("./double-dump");

                for (int i = 0; i < tableSize; i++) {
                    if (hashTableLinear.hashTable[i] != null){
                        dumpLinearDebug(hashTableLinear.hashTable[i], i, linearWriteFile);
                    }
                    if(hashTableDouble.hashTable[i] != null){
                        dumpDoubleDebug(hashTableDouble.hashTable[i], i, doubleWriteFile);
                    }
                }
                linearWriteFile.close();
    ...
    private static void dumpLinearDebug(HashObject hashObject, int index, FileWriter writeLinearFile){
        try {
    ...
    private static void dumpDoubleDebug(HashObject hashObject, int index, FileWriter writeDoubleFile){
        try {
