...
#
# JDBC_DATABASE_URL format for connecting from a virtual machine (or Docker) to a local database:
# jdbc:postgresql://[host]:[port]/[databaseName]?user=[username]&password=[password]
#
# JDBC_DATABASE_URL format for connecting from a virtual machine (or Docker) to the remote Heroku Postgres database:
...
java -DJDBC_DATABASE_URL="jdbc:postgresql://localhost:5432/postgresondockerdb?user=postgresondockeruser&password=postgresondockerpassword" -jar target/java-getting-started-1.0.jar
