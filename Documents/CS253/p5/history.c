...
void print_history(int firstSequenceNumber){ //Prints the history to stdout
	...
	if(startingIndex == 0){
		bottomNumberOfHistory = 0;
	}
	else{
		bottomNumberOfHistory = numberOfLastCommand - 9;	
	}
    while (loops < sizeOfCommandHistory){
        fprintf(stdout, "%d. [%d] %s\n", loops+bottomNumberOfHistory+1, commandHistoryArray[(startingIndex+loops)%10]->exitStatus, commandHistoryArray[(startingIndex+loops)%10]->cmd);
        loops++;
    }
}
