...
static void put(Rep r, End e, Data d) {
  ...
  newNode->data = d;
  newNode->np[e] = NULL; // I don't think this is necessary, but doing it anyways cause C is problematic
  ...
  if(r->len == 0){
    ...
    //Reassign the end value to be the new node 
    r->ht[e] = newNode;
  }
  r->len++;
}
...
//Will need to free the data after call due to the data being allocated
...
//Will need to free the data later due to having to malloc it in this method
static Data get(Rep r, End e) {
  //Data* ret = (Data*)malloc(sizeof(r->ht[e]->data));
...
//Search Head to Tail
  ...
  if(nodeIndex->data != d){
    ...
    //Data* ret = malloc(sizeof(nodeIndex->data));
    Data ret = nodeIndex->data;
    free(nodeIndex);
