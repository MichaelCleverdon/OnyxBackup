...
static ssize_t write(struct file *filp,
    ...
    if(file->changeSeparators == 1){
          kfree(file->separators);
          file->separators = (char*)kmalloc(sizeof(str)+1, GFP_KERNEL);
          if (!file->separators) {
            printk(KERN_ERR "%s: kmalloc() failed when setting separators in write\n",DEVNAME);
            return -ENOMEM;
          }
          strncpy(file->separators,str, len);
          file->changeSeparators = 0;
          return strlen(file->changeSeparators);
    }
    ...
    while(stringLength < len){
        int i = strcspn(str, file->separators);
        char* token;
        ...
        str += i+1; //Move the string to the not-yet-parsed section
        stringLength += i+1; //+1 for the separator we removed
        tokenArrayLength++;
    }
    stringTokens[tokenArrayLength] = NULL;
    ...
    Token** tokenArray = malloc(sizeof(*tokenArray) * (tokenArrayLength+1));//+1 for the null termination entry at the end
    int i = 0;
